...
class ItemBasedCF:  
    ...
    def splitData(self,data=None,k=3,M=10,seed=10):  
        ...
        for user,item,record in self.data:  
            ...
            self.traindata[item][user] = record #全量训练
            '''
        ...
        self.test = []
        file = open("testData1.csv",'r')  
        for line in islice(file, 1, None): #file.readlines():  
            userid, itemid, record = line.split(',')  
            self.test.append((userid,itemid,float(record)))

        for user,item,record in self.test:  
            self.testdata.setdefault(user,{})  
            self.testdata[user][item] = record #测试数据
    ...
    def recommend(self,user,train = None, k = 10,nitem = 5):  
        ...
        for i,pi in ru.items():
            print(sorted(self.itemSim[i].items(), key = lambda x:x[1], reverse = True)[0:k])
...
if __name__ == "__main__":  
    ...
    ibc=ItemBasedCF(os.getcwd()+'\\trainingData.csv')#初始化数据  
